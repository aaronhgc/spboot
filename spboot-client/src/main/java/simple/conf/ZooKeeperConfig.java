...
public class ZooKeeperConfig {
	...
	private void startZooKeeper() {
		...
		for (String instanceName : currChilds) {
			System.out.println("instanceName" +serviceName+ instanceName);
	...
	private UserService.Client createUserService(String serviceInstanceName) {
		String[] ip = serviceInstanceName.split("-");
		TSocket transport = new TSocket(ip[1], Integer.parseInt(ip[2]));
		TTransport transport2 = new TFramedTransport(transport);
		UserService.Client client=new UserService.Client(new TBinaryProtocol(transport2));
		try {
			transport2.open();
